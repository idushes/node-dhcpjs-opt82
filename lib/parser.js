    ...
    while (code != 255 && offset < msg.length) {
        ...
        switch (code) {
            ...
            case 82: {          // relayAgentInformation           (RFC 3046)
                ...
                p.opt82 = getOpt82(p.options.relayAgentInformation);
                break;
            }
...
function getOpt82(relayAgentInformation) {
    if ( relayAgentInformation && relayAgentInformation.agentCircuitId && relayAgentInformation.agentRemoteId) {
        if (relayAgentInformation.agentRemoteId.length > 12) {
            relayAgentInformation.agentRemoteId = relayAgentInformation.agentRemoteId.substr(4);
        }
        var mac = relayAgentInformation.agentRemoteId.substr(0);
        var vlan = "";
        var port = "";
        if (relayAgentInformation.agentCircuitId.length > 12) {
            vlan = relayAgentInformation.agentCircuitId.substr(4,4);
            port = relayAgentInformation.agentCircuitId.substr(12,4);
        } else {
            vlan = relayAgentInformation.agentCircuitId.substr(4,4);
            port = relayAgentInformation.agentCircuitId.substr(10,2);
        }
        return {
