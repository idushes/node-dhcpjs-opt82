...
billing.getIP = function (m, cb) {
    ...
    } else if ("chaddr" in m && "requestedIpAddress" in m.options) {
        // Продление аренды по  mac + ip адресу
        query = "SELECT INET_NTOA(segment) as ip, (NULL) as tmp_ip FROM staff WHERE last_mac = x'"+m.chaddr.replace(/:/g,"")+"' " +
            "AND segment = INET_ATON('"+m.options.requestedIpAddress+"') LIMIT 1";
    } else {
